...
urlpatterns = [
    ...
    # API endpoints - must come before slug patterns
    path(
        "feed-api/", views.track_feed_api, name="track_feed_api"
    ),  # API endpoint for track feed
    path(
        "api/<slug:slug>/audio/", views.track_audio_api, name="track_audio_api"
    ),
