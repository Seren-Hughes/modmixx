...
BLOCK_LABELS = {
    "Explicit Nudity", "Nudity", "Sexual Activity", "Suggestive",
    "Violence", "Hate Symbols", "Hate", "Smoking", 
}
...
def _client():
    ...
    print(f"DEBUG: Using Rekog key: {rekog_key[:10] if rekog_key else 'None'}...")
...
def scan_image_bytes(image_bytes: bytes):
    ...
    try:
        ...
        for lbl in labels:
            ...
            if name in ["Pills", "Products", "Drugs & Tobacco"] and confidence >= 99:
                blocked = True
                break
            # Other serious content: 85% confidence
            elif name in ["Explicit Nudity", "Sexual Activity", "Violence", "Hate Symbols", "Smoking"] and confidence >= 85:
                blocked = True
                break
