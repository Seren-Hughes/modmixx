...
class ProfileForm(forms.ModelForm):
        ...
        if display_name:
            ...
            if re.search(r"<[^>]*>", display_name):
                raise ValidationError(
                    "HTML tags are not allowed in display name."
                )
            ...
            if len(display_name) > 100:
                raise ValidationError(
                    "Display name too long. Maximum 100 characters."
                )
        return display_name
    ...
    def clean_pronouns(self):
        ...
        if pronouns:
            ...
            if len(pronouns) > 50:
                raise ValidationError(
                    "Pronouns too long. Maximum 50 characters."
                )
        return pronouns
    ...
    def save(self, commit=True):
        ...
        if hasattr(self, "_moderation_failed") or hasattr(
            self, "_moderation_allowed"
        ):
            if getattr(self, "_moderation_failed", False):
                ...
                profile.moderation_status = (
                    "APPROVED"
                    if getattr(self, "_moderation_allowed", True)
                    else "REJECTED"
                )
                profile.moderation_labels = getattr(
                    self, "_moderation_labels", []
                )
